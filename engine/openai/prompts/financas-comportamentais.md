# financas-comportamentais.md (adaptado pra engine Gemini)

> **Nota de adaptação — 2026-05-12:** este arquivo foi copiado de `../../prompts/financas-comportamentais.md` (engine Satori) e mantém intacta a **voz editorial** e a **biblioteca de ângulos / temas seed**. O que muda na nova engine:
>
> - O bloco "Estrutura de saída (JSON)" da versão antiga referenciava templates Satori (`pradex-capa`, `pradex-conteudo`, etc) — **isso não vale mais**. O shape novo de output vive em [`system-roteirista.md`](./system-roteirista.md): `slides[].texto_overlay` + `texto_meta` + `sujeito_visual`, sem campo `template`.
> - A direção visual por slide agora é descrita em **prosa** (`sujeito_visual`) em vez de codificada em template fixo — o Nano Banana é quem renderiza a composição.
> - Tudo abaixo desta linha permanece como referência de **voz, ângulos editoriais e temas seed**. Quando bater conflito entre o JSON shape do antigo e o do `system-roteirista.md`, **vence o `system-roteirista.md`**.
